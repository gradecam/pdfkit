...
(function() {
  ...
  module.exports = {
    ...
    currentFontMetrics: function() {
      return {
        ascender: this._font.ascender / 1000 * this._fontSize,
        descender: this._font.descender / 1000 * this._fontSize,
        gap: this._lineGap,
    ...
    _rawFragment: function(text, x, y, options) {
      var addSegment, align, base, cacheKey, characterSpacing, commands, enableCache, encoded, flush, glyphString, hadOffset, i, j, last, name, positions, ref, ref1, scale, spaceWidth, textWidth, useCache, wordSpacing, words;
      text = ('' + text).replace(/\n/g, '');
      if (text.length === 0) {
        return;
      }
      ...
      if (enableCache) {
        if (!this._fontGlyphCache) {
          this._fontGlyphCache = {};
        }
        if (this._fontGlyphCache[cacheKey]) {
          useCache = true;
          for (i = j = 0, ref = text.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
            if (!this._fontGlyphCache[cacheKey][text[i]]) {
              useCache = false;
            }
          }
        } else {
          useCache = false;
          this._fontGlyphCache[cacheKey] = {};
        }
      }
      this.save();
      this.transform(1, 0, 0, -1, 0, this.page.height);
      ...
      if (useCache) {
        glyphString = ((function() {
          var m, ref1, results;
          results = [];
          for (i = m = 0, ref1 = text.length - 1; 0 <= ref1 ? m <= ref1 : m >= ref1; i = 0 <= ref1 ? ++m : --m) {
            results.push(this._fontGlyphCache[cacheKey][text[i]]);
          }
          return results;
        }).call(this)).join('');
        this.addContent("[<" + glyphString + "> 0] TJ");
      } else {
        ref1 = this._font.encode(text, options.features), encoded = ref1[0], positions = ref1[1];
        if (enableCache) {
          encoded.forEach((function(_this) {
            return function(val, idx) {
              return _this._fontGlyphCache[cacheKey][text[idx]] = val;
            };
          })(this));
        }
        scale = this._fontSize / 1000;
